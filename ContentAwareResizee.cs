        ...
        public void CalculateSeamsCost(int[,] energyMatrix, int Width, int Height, ref int minSeamValue, ref List<coord> seamPathCoord)
        {
            ...
            for (int i = 1; i < Height; i++)
            {
                for (int j = 0; j < Width; j++)
                {
                    ...
                    if (j < Width - 1 && dp[i - 1, j + 1] < min)
                        min = dp[i - 1, j + 1];

                    dp[i, j] = energyMatrix[i, j] + min;
                }
            }
            ...
            for (int i = Height - 1; i >= 0; i--)
            {
                ...
                if (i > 0)
                {
                    ...
                    int min = dp[i - 1, currentCol];
                    int nextCol = currentCol;

                    if (currentCol > 0 && dp[i - 1, currentCol - 1] < min)
                    {
                        min = dp[i - 1, currentCol - 1];
                        nextCol = currentCol - 1;
                    }

                    if (currentCol < Width - 1 && dp[i - 1, currentCol + 1] < min)
                    {
                        min = dp[i - 1, currentCol + 1];
                        nextCol = currentCol + 1;
                    }
